...
**********LOCALIZACION DEL ESTUDIANTE EN NOTAS
	    ...
	    IF lLocCodigo('cCodigoEst','NOT',cCodigoTes)
		  ...
		  cNotDef := cNotRec
	       ...
	       IF nNroPer >= 4
		  cPromed := SUBS(&cCamNotDef,(5*4)-3,4)
		  cPromed := VAL(cPromed)
		  cPromed := STR(cPromed,4,1)
	       ENDIF
...
**********LECTURA DE LA NOTA
	     ...
	     @ nNroFil,aNroCol[4];
		       ...
		       VALID {|oLeeGet|lValNota(oLeeGet,nValNot,VAL(cPromed),nNroPer)}
	     READ
...
FUNCTION lValNota(oLeeGet,nValNot,nNotDef,nNroPer)
...
*>>>>VALIDACION DE CONTENIDOS DE ARCHIVOS
       ...
       CASE nNroPer < 4 .AND. nNotDef # NIL .AND.;
	    nNota > 0 .AND. nNota <= nNotDef
